...
public class LongHashSet {
	...
	public LongHashSet() {
		table = new long[LongHashSet.DEFAULT_INITIAL_CAPACITY][LongHashSet.DEFAULT_LIST_SIZE];
	}
	...
	public LongHashSet(int initialCapacity) {
		...
		if (initialCapacity < LongHashSet.MINIMUM_CAPACITY) {
			initialCapacity = LongHashSet.MINIMUM_CAPACITY;
		} else if (initialCapacity > LongHashSet.MAXIMUM_CAPACITY) {
			initialCapacity = LongHashSet.MAXIMUM_CAPACITY;
		}

		table = new long[initialCapacity][LongHashSet.DEFAULT_LIST_SIZE];
	}
	...
	public boolean add(long value) {
		...
		if (++list[0] == list.length) {
			...
			// larger.
			if (numEnlarged >= table.length >>> 4 && table.length < LongHashSet.MAXIMUM_CAPACITY) {
				// Undo change;
				--list[0];
				...
				numEnlarged += list.length / LongHashSet.DEFAULT_LIST_SIZE - 1;
			}
		}
	...
	private void growTable() {
		...
		table = new long[oldLength << 2][LongHashSet.DEFAULT_LIST_SIZE];
