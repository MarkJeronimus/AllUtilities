...
public class ImageConvolutionFlatSymmetricFilter extends ImageFilter {
	 ...
	 */
	public static ImageConvolutionFlatSymmetricFilter designGaussianBlur(float stDeviation, float precision,
	                                                                     int maxRadius) {
		double denominator = -0.5 / (stDeviation * stDeviation);

		int idealR = 0;
		do {
			idealR++;
		} while (Math.exp(idealR * idealR * denominator) > precision);
		idealR--;
