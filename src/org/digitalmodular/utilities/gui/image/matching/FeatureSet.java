 ...
 * THE SOFTWARE.
 */
package org.digitalmodular.utilities.gui.image.matching;
...
public class FeatureSet extends ArrayList<MatchFeature> {
	...
	public Vector2i getBestMatch() {
		...
		for (int i = length - 1; i > 0; i--) {
			MatchFeature first = super.get(i);
			for (int j = i - 1; j >= 0; j--) {
				double match = first.match(super.get(j));
				if (bestMatch > match) {
					bestMatch = match;
					best.set(i, j);
				}
	...
	public Vector2i getBestMatchUnMarked() {
		...
		for (int i = length - 1; i > 0; i--) {
			...
			for (int j = i - 1; j >= 0; j--) {
				MatchFeature second = super.get(j);
	...
	public void matchAndSort(MatchFeature matchItem) {
		...
		for (int i = length - 1; i >= 0; i--) {
			MatchFeature other = super.get(i);
		...
		// Store scores.
		for (int i = length - 1; i >= 0; i--) {
			super.get(i).setSortValue(matches[i]);
		}
