...
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageImageFunctions;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageImageFunctionsComponent;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageImageImageFunctions;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageImageImageFunctionsComponent;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageImageVectorFunctions;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageScalarFunctions;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageScalarFunctionsComponent;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixImageVectorFunctions;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixQueryFunctions;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixQueryFunctionsComponent;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixUnaryFunctions;
import org.digitalmodular.utilities.gui.image.function.ImageMatrixUnaryFunctionsComponent;
...
public class ImageMatrixFloat extends AbstractImageMatrix {
	...
	/**
	 * Create an empty {@link ImageMatrixFloat} object with a given size and number of color components.
	 *
	...
	/**
	 * Create an {@link ImageMatrixFloat} from a {@link BufferedImage}.
	 *
	...
	/**
	 * Create a copy (clone) of an {@link ImageMatrixFloat}, including it's data.
	 */
		...
		switch (image.getType()) {
			case BufferedImage.TYPE_INT_RGB: // 1
				...
				for (int y = border; y < endY; y++) {
					...
					for (x = border; x < endX; x++) {
						int c = intRaster[p++];
						row0[x] = (c >> 16 & 0xFF) / 255f;
						row1[x] = (c >> 8 & 0xFF) / 255f;
						row2[x] = (c & 0xFF) / 255f;
					}
				}
				break;
			...
			case BufferedImage.TYPE_3BYTE_BGR: // 5
				...
				for (int y = border; y < endY; y++) {
					...
					row2 = matrix[2][y];
					for (x = border; x < endX; x++) {
						row2[x] = (byteRaster[p++] & 0xFF) / 255f;
						row1[x] = (byteRaster[p++] & 0xFF) / 255f;
						row0[x] = (byteRaster[p++] & 0xFF) / 255f;
					}
				}
				break;
			...
			case BufferedImage.TYPE_USHORT_GRAY: // 11
				...
				for (int y = border; y < endY; y++) {
					row0 = matrix[0][y];
					for (x = border; x < endX; x++) {
						row0[x] = (int)((shortRaster[p++] & 0xFFFF) / 65535f);
					}
				}
				break;
			default:
		...
		switch (numComponents) {
			case 1:
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						matrix[0][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
			...
			case 2:
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						matrix[0][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
						matrix[1][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
			...
			case 3:
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						matrix[0][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
						matrix[1][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
						matrix[2][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
			...
			case 4:
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						...
						matrix[1][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
						matrix[2][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
						matrix[3][border + y][border + x] = (data[p += planeStride] & 0xFF) / 255f;
