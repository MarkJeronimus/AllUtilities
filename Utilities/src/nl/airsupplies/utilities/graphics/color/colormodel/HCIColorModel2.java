 ...
 */
// Created 2025-06-01
public class HCIColorModel2 extends ColorModel {
	@Override
	public Color3f fromRGB(Color3f color) {
		...
		if (chroma < 1.0e-7f) {
			return new Color3f(0.0f, 0.0f, max);
		}

		float hue = rgb2hueInternal(color.r, color.g, color.b, min, max, chroma);
		float sat = chroma / (1.0f - Math.abs(min + max - 1.0f));

		float intensity = ColorUtilities.toSRGB(ColorUtilities.getPerceptualLuminosity(color.r, color.g, color.b));
		return new Color3f(hue, sat, intensity);
	}
