...
public class IntHashMap<V> implements Serializable {
	...
	public IntHashMap() {
		table = new Entry[IntHashMap.DEFAULT_INITIAL_CAPACITY][IntHashMap.DEFAULT_LIST_SIZE];
	}
	...
	public IntHashMap(int initialCapacity) {
		...
		if (initialCapacity < IntHashMap.MINIMUM_CAPACITY) {
			initialCapacity = IntHashMap.MINIMUM_CAPACITY;
		} else if (initialCapacity > IntHashMap.MAXIMUM_CAPACITY) {
			initialCapacity = IntHashMap.MAXIMUM_CAPACITY;
		}

		table = new Entry[initialCapacity][IntHashMap.DEFAULT_LIST_SIZE];
	}
	...
	public boolean put(int key, V value) {
		...
		if (position == list.length) {
			...
			// larger.
			if (numEnlarged >= table.length >>> 4 && table.length < IntHashMap.MAXIMUM_CAPACITY) {
				growTable();
			...
			numEnlarged += list.length / IntHashMap.DEFAULT_LIST_SIZE - 1;
		}
	...
	private void growTable() {
		...
		table = new Entry[oldLength << 2][IntHashMap.DEFAULT_LIST_SIZE];
