...
/**
 * A Layout Manager which creates a basic table of components. The widths of the cells are specified as a ratio array of
 * {@code int}. The relative widths of the rendered columns will always follow these ratios.<br>
 * <p>
 * The preferred widths of the columns will be either the sum of the columns with configured widths as pixels, or the
 * configured ratios stretched equally in such a way that no single component is narrower than it's own preferred width,
 * whichever is larger, added to the sum of the gap widths. When the width of the parent container is larger, the cells
 * are simply stretched linearly according to the configured ratios.
 * <p>
 * A special case is when a ratio is specified as zero. In these cases, each column specified as zero will get such a
 * preferred width that no single component inside that column is narrower than it's own preferred width. These columns
 * do stretch, however, when the width of the parent container is larger than the preferred width, everything is
 * stretched linearly.
 * <p>
 * For example, a {@code TableLayout} with column widths {@code {5, 2}} filled with equal components with a
 * width of 100 pixels will generate a layout with preferred column widths of 250 and 100 pixels (ratio 5:2). Setting
 * this layout to a container with an absolute width of 700 pixels will force the table columns to 500 and 200 pixels
 * wide.<br>
 * <p>
 * Another example. A {@code TableLayout} with column widths {@code {0, 200, 0, 120}} filled with different
 * components for each column with relative widths of 30, 100, 50 and 100 pixels will generate a layout with preferred
 * column widths of 30, 200, 50 and 120 pixels (ratio x:200:x:120). Setting this layout to a container with an absolute
 * width of 800 pixels will force the table columns to 60, 400, 100 and 240 pixels wide.
 *
...
public class TableLayout implements LayoutManager, Serializable {
	 ...
	 *
	 * @param hgap    the horizontal space between all columns, in pixels
	 * @param vgap    the vertical space between all rows, in pixels
	 * @param columns the column-width ratios
	 * @throws IllegalArgumentException when the array is null, the array length is zero or one of the array
	 *                                  elements is
	 *                                  negative
	 */
	...
	/**
	 * Returns the column width ratios. The returned array is a direct reference to the contained column array, so care
	 * must be taken not to alter any elements to illegal values (negative values).
	 *
	...
	public String toString() {
		return getClass().getSimpleName() + "[columns=" + columns + ", hgap=" + hgap + ", vgap=" + vgap + ']';
	}
}
